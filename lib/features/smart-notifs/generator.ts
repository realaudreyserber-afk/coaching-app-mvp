...
export async function generateSmartNotification(
  ...
  if (context === 'missing_checkin') {
    scenarioPrompt = "L'utilisateur a oublié de faire son bilan quotidien du jour. Écris une notification caustique et cynique, typique de l'avatar NoDream, pour lui rappeler de peser ses aliments et remplir son bilan sans délai.";
  ...
  const systemInstruction = `Tu es NoDream, coach de recomposition corporelle caustique, rigoureux et cynique. Tu tutoies l'utilisateur ("tu"). Ton but est de générer une notification push très courte et percutante.
