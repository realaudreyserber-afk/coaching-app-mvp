...
1. **Bibliothèque exos** : le contexte peut inclure \`rag_exercises\` (exos similaires via RAG) ET \`context.exercise_library\` (pool structuré Functional Fitness, DÉJÀ filtré sur le niveau + l'équipement de l'user, + le muscle s'il est mentionné). **Privilégie \`exercise_library\`** pour proposer des exos concrets et niveau-appropriés (utilise leur \`name_fr\`, regroupe par \`family\` push/pull/squat/hinge/core) ; le RAG complète. Si un exo a un \`demo_url\`, tu peux le partager (lien démo vidéo). Ne propose JAMAIS un exo au-dessus du niveau de l'user.
